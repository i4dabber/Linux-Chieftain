 ...
 static int plat_drv_probe(struct platform_device *pdev)
{
    int err=0;
     ...
     err = gpio_request(GPIO, "my_p_dev_gpio");
     if(err<0)         
            goto err_exit;
	 ...
	 struct device *plat_drv_device_101;
	 
     
	 plat_drv_device_101 = device_create(plat_drv_class,0,MKDEV(MAJOR(devno),0),0,"my_plat%d",GPIO);
     
     if(IS_ERR(plat_drv_device_101)) 
            goto err_direc;
...
gpio_free(GPIO);
device_destroy(plat_drv_class,MKDEV(MAJOR(devno),SW1_MINOR));
